...
static void send_seek_event (CustomData *data) {
  ...
  /* 使用gst_event_new_seek()创建Seek事件。它的参数基本上是，新的速率，新的开始位置和新的停止位置。无论播放方向如何，
   * 起始位置必须小于停止位置，因此这两个播放方向的处理方式不同。*/
  if (data->rate > 0) {
  ...
  /* Send the event. 使用gst_element_send_event()将新事件发送到选定的接收器 */
...
static gboolean handle_keyboard (GIOChannel *source, GIOCondition cond, CustomData *data) {
  ...
  case 'n':
    ...
    /* gst_event_new_step()创建一个新步骤事件，该事件的参数基本上指定要跳过的数量(本例中为1帧)和新速率(我们不会更改) */
